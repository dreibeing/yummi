...
- `scripts/meal_builder.py` now requires a predefined scope (`--predefined-dir data/archetypes/predefined/<slug>`) so each meal lives under `data/meals/<slug>/<archetype_uid>/` with run logs in `data/meals/runs/<slug>/`. Validation fills any missing required tags from the archetype defaults—plus it enforces curated ingredient usage per UID—and `--archetype-uid all` can batch one meal per archetype inside the scope once `archetypes_combined.json` + curated ingredients are present.
