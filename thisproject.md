...
- Preference onboarding now persists to Fly: `/v1/preferences` accepts Clerk-authenticated PUT/GET calls, stores normalized preference tags in `user_preference_profiles`, and the Expo flow surfaces the sync timestamp after completion. The thin-slice “Diet & Ethics” and “Avoidances & Allergens” screens now use single-tap toggle controls (mirroring “Who Are We Feeding?”), default to “No restrictions”/“No allergen avoidance” on the first run, and automatically clear the default whenever other options are chosen (and vice versa).
...
- New Ingredients surface: tapping “Next” on the meal home screen pushes a dedicated Ingredients view that aggregates the products for all currently active meals, sorts them alphabetically, and provides inline quantity controls, per-item pricing, an estimated basket total, plus CTAs for generating a shopping list or sending the items to the Woolworths cart filler. Each ingredient row now renders the preferred product image (pulled from `linkedProducts[].imageUrl`) or a branded placeholder initial before exposing the quantity steppers (`thin-slice-app/App.js:2194`).
- Shopping list build now flows through `/v1/shopping-list/build`: the thin-slice client confirms the free-use modal, posts the selected meals (including `final_ingredients` + linked SKUs), and the FastAPI service aggregates quantities before calling GPT-5 Mini (low reasoning effort, 12k output cap). Ingredients with a single product tied to a single meal are auto-resolved, and every other ingredient gets its own LLM request so the model only sees the relevant context; responses and auto-assignments merge into pantry vs pickup classifications plus per-group pricing metadata (unit price + minor-units) which the Expo ingredients screen uses to render currency badges and basket totals alongside the “preparing your shopping list…” spinner and retry affordances.
- Shopping list prompt + hydration details: `_build_system_prompt`/`_build_user_prompt` now serialize meal summaries, ingredient-group rollups, and a strict JSON schema that forces one entry per group, pantry vs pickup classification, and explicit `product_selections[{product_id,packages}]` choices. The model is instructed to choose only from resolver-backed SKUs and to keep pantry entries at zero packages, preventing hallucinated products (`yummi-server/app/services/shopping_list.py:353`). Each `ShoppingListProductSelection` now carries an `imageUrl`, wired all the way from resolver metadata through normalization/hydration helpers to the API schema so the client can display accurate pack art even when GPT selects a classified product option (`yummi-server/app/schemas.py:265`, `yummi-server/app/services/shopping_list.py:229`).
- The “Add to Woolworths Cart” CTA on the Ingredients screen now fills the runner basket automatically: ingredient quantities feed straight into the thin-slice runner order queue, skipped SKUs are surfaced before handoff, and the redesigned Woolworths checkout WebView matches the rest of the app while returning users to Welcome once they tap Finish.
...
- The new aggregation CLI (`scripts/meal_aggregate_builder.py`) walks all per-archetype directories, normalizes tags, and emits a single manifest at `resolver/meals/meals_manifest.json` (and optional Parquet rows when `pyarrow` is installed). Each manifest carries `schema_version`, `manifest_id`, stats, warnings, and every archetype -> meal relationship required by the thin slice. Latest publish: `manifest_id=meals_20251124T072500Z` (10 archetypes, 18 meals) built from `data/meals/none_family_none` and deployed with the server.
...
- Thin-slice + extension already read `/v1/meals*`; after each manifest refresh, re-run the onboarding/exploration flow so new meal IDs are persisted for users.
