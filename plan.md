...
3. Run `python scripts/predefined_archetype_aggregator.py` so every scope has `archetypes_combined.json`, finish meal coverage for each archetype you plan to ship (`scripts/meal_builder.py --archetype-uid <uid>`), then regenerate `resolver/meals/meals_manifest.json` so `/v1/meals*` exposes the full portfolio before product QA begins.
